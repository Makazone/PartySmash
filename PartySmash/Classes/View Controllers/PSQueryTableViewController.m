...
@interface PSQueryTableViewController () {
    ...
    UIActivityIndicatorView *_loadMoreControl;
}
...
@implementation PSQueryTableViewController
...
- (void)objectsDidLoad:(NSError *)error {
    ...
    [_loadMoreControl stopAnimating];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (currentOffset <= 0 || !self._shouldShowPaginationCell) { return; }
...
//    NSLog(@"currentOffset = %f", currentOffset);
    ...
    if (deltaOffset <= 0) {
        [_loadMoreControl startAnimating];
        [self loadNextPage];
    }
}
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    if (self._shouldShowPaginationCell) {
        _loadMoreControl = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
        _loadMoreControl.frame = CGRectMake(0, 0, 320, 48);
