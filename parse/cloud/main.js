...
Parse.Cloud.define("updateUserPhotosOnDev", function(request, response) {
    ...
    .then(function(image) {
        ...
        var base64 = buffer.toString("base64");
        var cropped = new Parse.File(photoName, { base64: base64 });
        return cropped.save();
    }).then(function(photo) {
        var user = new Parse.User();
        user.id = request.params.userId;

        user.set(photoName, photo);
        user.save().then(function(user) { response.success("OK"); });
    });
});
